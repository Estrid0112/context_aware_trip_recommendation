...
public class Sequence {
    ...
    Map<String,Float> getResultStats(Sequence sequence, String[] stats) {
        ...
        for (int i = 0; i < stats.length; i++) {
            String stat = stats[i];
            switch (stat){
                case "precision": resultStats.put("precision", precision);
                break;
                case "recall": resultStats.put("recall", recall);
                break;
                case "F1": resultStats.put("F1", getFScore(precision, recall));
                break;
                case"precision_2": resultStats.put("precision_2", precision_2);
                break;
                case "recall_2": resultStats.put("recall_2", recall_2);
                    break;
                case "F1_2": resultStats.put("F1_2", getFScore(precision_2,recall_2));
                    break;
            }
        }
        return resultStats;
